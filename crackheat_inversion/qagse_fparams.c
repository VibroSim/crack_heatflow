/* qagse_fparams.f -- translated by f2c (version 20160102).
...
/* Subroutine */ int qagse_(E_fp f, real *fp1, real *fp2, real *fp3, real *
	fp4, real *fp5, real *fp6, real *fp7, real *fp8, real *a, real *b, 
...
/*            fp3    - real  parameter #3 for f */
/*            fp4    - real  parameter #4 for f */
/*            fp5    - real  parameter #5 for f */
/*            fp6    - real  parameter #6 for f */
...
/*           ----------------------------------- */
    ...
    oflow = r1mach_(&c__2);
    ierro = 0;
    qk21_((E_fp)f, fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8, a, b, result, 
...
/*           error estimate. */
	...
	b2 = blist[maxerr];
	erlast = errmax;
	qk21_((E_fp)f, fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8, &a1, &b1, &
		area1, &error1, &resabs, &defab1);
	qk21_((E_fp)f, fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8, &a2, &b2, &
...
/* Subroutine */ int qk21_(E_fp f, real *fp1, real *fp2, real *fp3, real *fp4,
	 real *fp5, real *fp6, real *fp7, real *fp8, real *a, real *b, real *
	result, real *abserr, real *resabs, real *resasc)
{
...
/*           the integral, and estimate the absolute error. */
    ...
    resg = 0.f;
    fc = funct_(&centr, fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8);
    resk = wgk[10] * fc;
    *resabs = dabs(resk);
    for (j = 1; j <= 5; ++j) {
	jtw = j << 1;
	...
	r__1 = centr - absc;
	fval1 = funct_(&r__1, fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8);
	r__1 = centr + absc;
	fval2 = funct_(&r__1, fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8);
	fv1[jtw - 1] = fval1;
	fv2[jtw - 1] = fval2;
	fsum = fval1 + fval2;
	resg += wg[j - 1] * fsum;
